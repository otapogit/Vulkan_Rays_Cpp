...
namespace autis {
	...
	public:
		...
		*/
		void read(void *dst, size_t size, uint32_t offset);
		...
		/** Escribe en una parte del B.O. vinculado los datos apuntados por src. Se
		 ...
		 excepción  */
		void write(const void *src, uint32_t size, uint32_t offset);
		...
		*/
		...
		\return un puntero en memoria de CPU
		*/
		//void *map(uint32_t offset, uint32_t size, GLbitfield access);
		/**
		...
		*/
		...
		\param format el formato de la memoria apuntada por data (p.e., GL_RED, GL_RG, GL_RGBA, GL_
		\param type el tipo de datos de la memoria apuntado por data (
