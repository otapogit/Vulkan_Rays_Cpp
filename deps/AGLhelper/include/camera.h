...
namespace autis {
	...
	public:
		...
		*/
		void render(GLContext &ctx) const;
		/**
...
#undef near
#undef far
// see https://stackoverflow.com/questions/8948493/visual-studio-doesnt-allow-me-to-use-certain-variable-names
		...
		/**
		Devuelve las coordenadas en el sistema del mundo de las coordenadas en el sistema de clip
		\param pos vértice en el espacio de clip (normalmente normalizado entre -1 y 1, pero puede
		estar escalado por la w)
		*/
		glm::vec3 fromClipToWorld(const glm::vec4& pos) const;

		/**
		Devuelve el rayo que parte de la cámara y pasa por el centro del píxel x, y
		\warning El píxel viene dado en el sistema del S.O. (origen en la esquina superior 
		izquierda)
		*/
		Ray screenPointToRayUL(int32_t x, int32_t y, int32_t width, int32_t height) const;

		/**
		 * @brief Ajusta los planos near y far para capturar toda la bounding box
		 * @param bbox 
		*/
		void adjustNearFar(const BoundingBox& bbox);
	protected:
		glm::mat4 viewMatrix, projMatrix;
		mutable glm::mat4 inverseViewProj{ 1.0f };
		mutable bool inverseUpdated{ false };
